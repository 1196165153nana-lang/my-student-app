...
# undo_cache 结构： {"op_type":"add_record|del_record", "table":"xxx", "record_id":"xxx", "data":{}}
...
# ==========【账目&明细｜层级：顶部月份+总指标，下方再左右分栏】==========
    ...
    if st.session_state["undo_cache"] is not None:
        undo_input = st.number_input("输入数字1，撤销上一步操作", min_value=0, max_value=1, step=1, key="undo_in_account")
        if undo_input == 1:
            do_undo()
...
# --- 模块：快速录课【新增后支持撤销，撤销会删掉刚录入的这节课】---
    ...
    # 撤销数字输入框
    if st.session_state["undo_cache"] is not None:
        undo_input = st.number_input("输入数字1，撤销上一步操作", min_value=0, max_value=1, step=1, key="undo_in_input")
        if undo_input == 1:
            do_undo()
...
# --- 模块：学生档案【新增学员、删除学员都支持撤销】---
    ...
    # 撤销数字输入框
    if st.session_state["undo_cache"] is not None:
        undo_input = st.number_input("输入数字1，撤销上一步操作", min_value=0, max_value=1, step=1, key="undo_in_student")
        if undo_input == 1:
            do_undo()
