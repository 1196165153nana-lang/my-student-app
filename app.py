...
# 【关键修复】定义哪些内容触发复习提醒
# 增加了 "旧数据补录" 和 "导入"，确保你搬家过来的历史数据能正常提醒
...
menu = st.selectbox("📌 切换功能模块", ["🔍 单词复习提醒", "📝 课时快速录入", "📊 工资财务统计", "👤 学生库管理", "📜 历史记录明细", "📄 导出21天表", "📥 批量导入旧数据"])

# --- 模块 1：单词复习提醒 (已修复旧数据不显示问题) ---
if menu == "🔍 单词复习提醒":
    st.subheader("🔍 单词复习清单")
    ...
    if not r_df.empty:
        ...
        mask = r_df['dt'].isna()
        if mask.any(): r_df.loc[mask, 'dt'] = pd.to_datetime(r_df.loc[mask, '学习日期']).dt.date
        ...
        if reminders:
            st.error(f"🚨 今日共有 {len(reminders)} 位学员需复习单词")
...
# --- 其余模块代码保持逻辑同步 ---
    ...
    if not s_df.empty:
        ...
        with st.form("input_f"):
            name = st.selectbox("👤 学员", active_s)
            date = st.date_input("📅 日期", datetime.date.today())
            content = st.selectbox("📚 内容", LEARN_CONTENTS)
            hour = st.selectbox("⏰ 课时", options=HOURS_OPTIONS, index=1)
    ...
    if not r_df.empty:
        ...
        st.metric("应发总工资", f"¥{m_df['小计'].sum():,.0f}")
        
        st.markdown("#### 👤 学生个人课时汇总")
        def merge_c(c):
        ...
        u_names = s_order['姓名'].unique().tolist()
        final.insert(0, '序号', final['姓名'].map({n: i+1 for i, n in enumerate(u_names)}))
    ...
    with st.expander("➕ 添加新成员"):
        with st.form("add"):
            n = st.text_input("学生姓名"); s = st.selectbox("状态", STATUS_OPTIONS)
    ...
    if not s_df.empty:
        ...
        t = st.selectbox("🗑️ 选择要删除的学生", ["请选择"] + s_df['姓名'].tolist())
    ...
    if not all_r.empty:
        all_r['日期'] = pd.to_datetime(all_r['学习日期'], unit='ms', errors='coerce').dt.strftime('%Y-%m-%d')
